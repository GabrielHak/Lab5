...
hanoi_t hanoi_init(unsigned int disk_count) {
    ...
    hanoi->target = NULL;
    /**
    * Conditional jump or move depends on uninitialised value(s), because hanoi->source was uninitialized
    * This error is solved by doing :
    * 1) hanoi->source = NULL 
    * 2) or using calloc(1, sizeof(struct _hanoi)) instead of malloc(sizeof(struct _hanoi))
    * beacuse calloc initializes all values to Zero or NULL and malloc just returns the memory address of 
    * reserved memory but uninitilized
    */
    hanoi->source = NULL;
    /* ----------------------------------------------------------------------------------------------------*/
...
hanoi_t hanoi_destroy(hanoi_t hanoi) {
    assert(hanoi != NULL);
    /* Faltaba liberar los stacks dentro de la estructura */ 
